export function getElementIds() {
    return {
        ...
        tenTruong: document.getElementById("tenTruong"),
...
export function viewTbody(data, onEdit, onDelete) {
    ...
    for (const item of data) {
        ...
        row.innerHTML = `
            <td style="text-align: center;">${item.ma_truong}</td>
            <td style="text-align: center;">${item.ten_truong}</td>
...
export function fillForm(item) {
    const { maTruong, tenTruong } = getElementIds();
    maTruong.value = item.ma_truong;
    tenTruong.value = item.ten_truong;
